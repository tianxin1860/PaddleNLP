#bash commands/train_all.sh tnews 0 &
#bash commands/train_all.sh bustm 1 &
#bash commands/train_all.sh cluewsc 4 &
#bash commands/train_all.sh iflytek 5 &
#bash commands/train_all.sh eprstmt 6 &
#bash commands/train_all.sh csl 7 &

#bash commands/train_all.sh chid 3 &
#bash commands/train_all.sh ocnli 3 &

#bash commands/train_all.sh bustm 1 &

#bash commands/train_all.sh iflytek 7 &
#bash commands/train_all.sh csldcp 6 &
#bash commands/train_all.sh chid 5 &
bash commands/train_all.sh csl 4 &
